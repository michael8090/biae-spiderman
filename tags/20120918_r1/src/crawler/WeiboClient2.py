...
#support the sleep and re-try   
    ...
    def _fetchMultiplePages(self, iAPI, iParams, pagination_handler = None):


        if not iParams.has_key('count'):
            iParams['count'] = 200
        
        if paginationHandler is None:
            if self.mPagingAPIs[iAPI] == 'cursor':
                pagination_handler = cursor_pagination_handler
            else:
                pagination_handler = page_num_pagination_handler
